...
所有新用户都会添加为该帐户的成员。您可以为被邀请者添加组织和空间角色，以在 {{site.data.keyword.Bluemix_notm}} 中启用特定视图和许可权。缺省情况下，会向添加到组织的新用户（在本地或专用环境中除外）分配审计员组织角色。对于特定空间，可以选择向被邀请者分配管理员、开发者或审计员角色。一旦被邀请者接受邀请并加入 {{site.data.keyword.Bluemix_notm}}，您就可以在“用户”页面上编辑这些被邀请者的角色。
...
如果您有权分配基础架构角色，那么可以在邀请用户时，设置以下许可权： 
