...
可以在帐户的**用户**页面中，管理 {{site.data.keyword.Bluemix_notm}} Platform 和 Infrastructure 服务上的用户。要访问“用户”页面，请在 {{site.data.keyword.Bluemix_notm}} 菜单中，单击**管理** &gt; **帐户** &gt; **用户**。帐户所有者可对组织和空间执行所有操作，包括管理用户及其分配的角色。组织管理员和空间管理员也有权管理角色。
{:shortdesc}
...
Cloud Foundry 角色包含帐户中定义的组织和空间的访问许可权。Cloud Foundry 角色不会授予用户许可权，完成服务上下文中的操作。可以在组织级别分配以下角色：
...
|审计员| 组织审计员可以查看组织中的应用程序和服务内容。审计员还可以查看组织中的用户及其分配的角色，以及组织的配额。缺省情况下，所有用户都会在受邀请时被授予审计员角色。在用户接受邀请后，可以将此角色更新为管理员或记帐管理员。|
...
除了控制台中提供的角色的描述外，下表还提供了对于 IBM Containers 服务，分配有每个角色的用户可以执行的一些任务示例。  

**注**：目前，操作员角色不可用于 IBM Container 服务。包含的以下示例仅供参考。

| 角色| 操作描述| 示例操作|
|:-----------------|:-----------------|:-----------------|
| 查看者| 执行不会更改状态的操作；只读操作| <ul><li>列出集群</li><li>查看集群的详细信息</li></ul>|
| 编辑者| 执行会修改状态以及创建或删除子资源的操作|<ul><li>添加或除去工作程序节点</li><li>重新引导或重新装入工作程序节点</li><li>将服务绑定到集群</li></ul> |
| 操作员| 执行配置和运行资源所需的操作。| <ul><li>添加或除去工作程序节点</li><li>重新引导或重新装入工作程序节点</li><li>将服务绑定到集群</li></ul> |
| 管理员| 执行所有操作，包括管理访问控制的能力|<ul><li>除去集群</li><li>创建集群</li><li>更新用户访问策略</li><li>查看者、编辑者和操作员可以执行的所有操作</li></ul>|
